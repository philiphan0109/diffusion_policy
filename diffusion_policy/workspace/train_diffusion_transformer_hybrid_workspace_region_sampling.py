...
class TrainDiffusionTransformerHybridWorkspaceRegionSampling(BaseWorkspace):
    ...
    def run(self):
        ...
        region_expected_mass = None
        if hasattr(dataset, "_region_sampling_stats") and dataset._region_sampling_stats is not None:
            region_expected_mass = dataset._region_sampling_stats.get("expected_mass", None)
        ...
        region_cumulative_counts = np.zeros((3,), dtype=np.int64)
        region_cumulative_invalid = 0
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                for batch_idx in tqdm.tqdm(range(cfg.training.max_train_steps), desc=f"Training epoch {self.epoch}", leave=False, mininterval=cfg.training.tqdm_interval_sec):
                    ...
                    region_batch_counts = None
                    region_invalid_count = 0
                    if isinstance(batch, dict) and ("region_label" in batch):
                        region_labels = batch["region_label"]
                        region_labels = region_labels.reshape(-1).to(torch.int64)
                        binc = torch.bincount(region_labels, minlength=3)
                        if binc.shape[0] > 3:
                            region_invalid_count = int(binc[3:].sum().item())
                        binc3 = binc[:3]
                        region_batch_counts = binc3.detach().cpu().numpy().astype(np.int64)
                        region_cumulative_counts += region_batch_counts
                        region_cumulative_invalid += int(region_invalid_count)
                    ...
                    if region_batch_counts is not None:
                        step_log.update({
                            "sampling/region_batch_free": float(region_batch_counts[0]),
                            "sampling/region_batch_pre_contact": float(region_batch_counts[1]),
                            "sampling/region_batch_contact": float(region_batch_counts[2]),
                            "sampling/region_batch_invalid": float(region_invalid_count),
                            "sampling/region_cumulative_free": float(region_cumulative_counts[0]),
                            "sampling/region_cumulative_pre_contact": float(region_cumulative_counts[1]),
                            "sampling/region_cumulative_contact": float(region_cumulative_counts[2]),
                            "sampling/region_cumulative_invalid": float(region_cumulative_invalid),
                        })

                        cum_total = max(float(region_cumulative_counts.sum()), 1.0)
                        step_log.update({
                            "sampling/observed_free": float(region_cumulative_counts[0] / cum_total),
                            "sampling/observed_pre_contact": float(region_cumulative_counts[1] / cum_total),
                            "sampling/observed_contact": float(region_cumulative_counts[2] / cum_total),
                        })

                        if region_expected_mass is not None and len(region_expected_mass) == 3:
                            step_log.update({
                                "sampling/expected_free": float(region_expected_mass[0]),
                                "sampling/expected_pre_contact": float(region_expected_mass[1]),
                                "sampling/expected_contact": float(region_expected_mass[2]),
                            })
...
def main(cfg):
    workspace = TrainDiffusionTransformerHybridWorkspaceRegionSampling(cfg)
    workspace.run()
